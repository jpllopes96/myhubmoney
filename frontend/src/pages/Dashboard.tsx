...
export default function Dashboard() {
  ...
  // ✅ FILTER CORRIGIDO (ÚNICA MUDANÇA REAL)
  const filteredTransactions = useMemo(() => {
    ...
    // ✅ proteção TOTAL contra digitação parcial
    if (
      !startDate ||
      !endDate ||
      startDate.length < 10 ||
  ...
  const chartData = useMemo(() => {
    return filteredTransactions.reduce(
      (acc, t) => {
        const month = parseISO(t.date).toLocaleDateString("pt-BR", {
          month: "short",
        });
  ...
  const getSubtitle = () => {
    ...
    // ✅ proteção contra digitação parcial
    if (
      !startDate ||
      !endDate ||
      startDate.length < 10 ||
    ...
    try {
      ...
      return `De ${format(start, "dd/MM/yyyy")} até ${format(
        end,
        "dd/MM/yyyy",
      )}`;
    } catch {
  ...
  return (
    <PageLayout
      title={`Olá${user?.user_metadata?.full_name ? `, ${user.user_metadata.full_name.split(" ")[0]}` : ""}!`}
